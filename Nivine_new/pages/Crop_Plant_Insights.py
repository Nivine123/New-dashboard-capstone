...
with plant_left:
    possible_paths = [
        Path(__file__).resolve().parents[1] / "data" / "greenhouse_systems_cleaned.csv",
        Path(__file__).resolve().parents[1] / "greenhouse_systems_cleaned.csv",
        Path.cwd() / "data" / "greenhouse_systems_cleaned.csv",
        Path.cwd() / "greenhouse_systems_cleaned.csv",
    ]
    ...
    if plant_df is None:
        st.warning(
            "Full cleaned dataset file was not found, so the chart is using the currently filtered dashboard data."
        )
        plant_df = df.copy()
